...
__global__ void sobel_kernel(unsigned char* img, unsigned char* sobel_module, float* sobel_dir,
    unsigned char kernel_size, unsigned char radius, unsigned int sizeSM,
    short rows, short cols, unsigned char L2_norm);

__global__ void non_max_suppression_kernel(unsigned char* sobel_module, float* sobel_dir, unsigned char* out,
    unsigned int sizeSM, unsigned char radius, short rows,
    short cols);

__global__ void hysteresis_kernel(unsigned char* img_non_max_sup, unsigned char* out, unsigned char* sobel_module,
    unsigned int sizeSM, unsigned char radius, short rows,
    short cols, int low_threshold, int high_threshold);
...
__global__ void hysteresis_kernel(unsigned char* img_non_max_sup, unsigned char* out, unsigned char* sobel_module,
    unsigned int sizeSM, unsigned char radius, short rows,
    short cols, int low_tr, int high_tr){
...
void cannyGPU(unsigned char* img_h, unsigned char* out, short rows,
    ...
    unsigned char *out_non_max_device, *out_d;
    float *sobel_dir_d;
    unsigned char *img_d, *sobel_module_d;
    ...
    // Size of memory to allocate to store the image
    size = rows*cols*sizeof(unsigned char);

    //Memoery allocation on the device
    cudaMalloc((void**)&img_d, size);
    cudaMalloc((void**)&sobel_dir_d, rows*cols*sizeof(float));
    cudaMalloc((void**)&sobel_module_d, size);
    ...
    // Definition of the kernel weights
    kernel_weights k;
    init_kernel_weights(k, kernel_size);

    // Copy the weights to the constant memory
    cudaMemcpyToSymbol(const_weights, &k, sizeof(k));

    // Calculate the convolution radius
    ...
    // Sobel kernel
    sobel_kernel<<<num_blocks, num_threads_per_block, sizeSMbyte>>>(img_d, sobel_module_d, sobel_dir_d, kernel_size, radius, sizeSM, rows, cols, L2_norm);
    ...
    non_max_suppresion_kernel<<<num_blocks, num_threads_per_block, sizeSMbyte>>>(sobel_module_d, sobel_dir_d, out_non_max_device, sizeSM, radius, rows, cols);
    cudaDeviceSynchronize();
    // Hysteresis kernel
    sizeSMbyte = 2*(num_threads_per_block.x+3-1)*(num_threads_per_block.y+3-1)*sizeof(unsigned char);
    hysteresis_kernel<<<num_blocks, num_threads_per_block, sizeSMbyte>>>(out_non_max_device, out_d, sobel_module_d, sizeSM, radius, rows, cols, low_threshold, high_threshold);
    cudaDeviceSynchronize();
    ...
    cudaFree(out_d);
    cudaFree(img_d);
    cudaFree(sobel_module_d);
    cudaFree(sobel_dir_d);
}
